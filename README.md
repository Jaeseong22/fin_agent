...
# ⚠️ 프로젝트 구동 및 데이터 환경 안내 (Notice)
> **본 프로젝트는 '미래에셋 AI 페스티벌'에서 기획/개발을 시작하여, 대회 종료 후 LLM 에이전트 아키텍처와 데이터 파이프라인을 독자적으로 고도화하여 마무리한 오픈소스 금융 에이전트입니다.**
> 
> 시스템 구동을 위해서는 `yfinance` API 기반의 한국 주식 6개월 시계열 데이터가 적재된 데이터베이스(DB) 환경이 필요하며, 보안을 위해 저장소 내에 환경 변수(`.env`) 파일은 포함되어 있지 않습니다. 
> 하지만 **데이터 수집, DB 적재 파이프라인, LLM 금융 분석 에이전트의 Reasoning 및 실행 워크플로우**는 코드 레벨에 온전히 구현되어 있어 인프라 연결 시 즉시 구동 가능합니다.

# 🛠 기술 스택 및 활용 목적

## • AI / Agentic AI
- **LangChain / OpenAI GPT-4o (또는 사용하신 모델):** 금융 데이터를 해석하고, 투자 인사이트를 도출하는 에이전트의 판단 흐름(Reasoning Workflow) 제어
- **Prompt Engineering:** 금융 도메인 지식을 반영한 시스템 프롬프트 설계 및 Hallucination 억제

## • Data Pipeline & Database
- **yfinance API:** 한국 주식 시장의 실시간/시계열 데이터 수집
- **[사용하신 DB 이름, 예: PostgreSQL/MySQL]:** 6개월간의 시계열 누적 데이터 적재 및 관리 (시계열 로그 분석 환경 구축)
